#!/usr/bin/env bash
# Basic scuv workflow - Creating and using virtual environments
#
...
echo "=== scuv Basic Workflow Example ==="
echo

# 1. Check scuv installation
echo "1. Checking scuv installation..."
scuv --version
echo "✅ scuv is installed"
echo

# 2. Install Python 3.12 if not already installed
echo "2. Installing Python 3.12..."
if scuv list --pythons --json | grep -q "3.12"; then
...
if scuv list --json | grep -q "demo-project"; then
    echo "ℹ️  Environment 'demo-project' already exists, skipping"
else
    scuv create demo-project 3.12
...
(
    # Re-source scuv to trigger auto-activation
    eval "$(scuv init bash)"
...
echo "    To remove: scuv remove demo-project"
# scuv remove demo-project
echo
...
echo "  - Run: scuv use demo-project"
