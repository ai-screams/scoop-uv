...
mod tests {
    ...
    //
    // The `system` value in .scuv-version (or the global version file) is
    // a documented sentinel meaning "use the system
    ...
    fn version_check_treats_global_system_sentinel_as_ok() {
        with_temp_scoop_home(|temp| {
            // Global version file lives at <SCUV_HOME>/version (see
            ...
            let global = results
                ...
                .expect("version:global must run when ~/.scuv/version exists");
            assert!(
                global.is_ok(),
    ...
    // ==========================================================================
    // VersionCheck: only `.scuv-version` is a local version file — a
    // scoop-era `.scoop-version` in cwd is not read (0.16.0 dropped the
    // fallback), so doctor and VersionService::resolve() agree.
    // ==========================================================================

    /// Fails if a `.scoop-version` fallback is reintroduced in the local
    /// version-file lookup.
    #[test]
    #[serial]
    fn version_check_ignores_legacy_only_local_version_file() {
        with_temp_scoop_home(|temp| {
            let cwd_guard = TempDirCwdGuard::new();
            ...
            let results = VersionCheck.run();
            assert!(
                !results.iter().any(|r| r.id == "version:local"),
                "a legacy-only .scoop-version must not produce a version:local result, got {results:#?}"
            );
        });
    }
