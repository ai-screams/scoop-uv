...
> **Note**: `scuv completions <shell>` is generated by clap via `clap_complete`, but the
> completions built into the init scripts are hand-written per shell (`_scuv_complete()`
> in `bash.rs`, `complete -c scuv` lines in `fish.rs`, the `langs` array in `zsh.rs`) —
> those need updating by hand when a subcommand or option changes.
...
2. Register in `cli/commands/migrate/` (its `mod.rs`):
```rust
let sources = vec![
    // ... existing
    Box::new(MySource),
